...
public class DateUtil {
	...
	public static Date getIntervalYear(Date date, Boolean isBefore, int interval) {
		Calendar calendar = Calendar.getInstance(TimeZone.getDefault());
		calendar.setTime(date);
		calendar.add(Calendar.YEAR, isBefore ? -interval : +interval);
		return str2Date(date2Str(calendar.getTime(), "yyyy-MM-dd HH:mm:ss"), "yyyy-MM-dd HH:mm:ss");
	}
	...
	public static int getIntervalDate(Date beginDate,Date endDate){
		long begindate = beginDate.getTime();//得到毫秒数
		long enddate = endDate.getTime();
	    long intval = enddate-begindate;//两时间差的总毫秒数
	    int  min = (int)(intval/60000);//转化成分钟
		return min;//返回分钟
	}
